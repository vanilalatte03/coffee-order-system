...
- 총 11회 시도(최초 dispatch 1회와 최대 10회 재시도) 통합 테스트는 `MutableClock`, 결정적 jitter와 production과 같은 `runOneCycle` 경계를 사용한다. 매 cycle마다 `next_attempt_at`으로 시간을 전진해 짧고 제한된 Awaitility 조건 안에서 최종 `FAILED`를 검증한다.
