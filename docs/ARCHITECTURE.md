...
- 단일 기능의 상태 변경은 Service가 트랜잭션 경계를 소유한다.
- 여러 기능을 가로지르는 흐름은 유스케이스를 소유한 기능의 Facade가 Service를 조합한다. Facade는 Repository를 직접 주입하지 않는다.
- 주문·포인트·원장·Outbox·멱등 결과는 `OrderFacade`가 하나의 유스케이스, 트랜잭션 범위와 지갑 락 순서로 조정한다. 실제 트랜잭션 실행과 유니크 충돌 후 재조회 경계는 `IdempotencyExecutor` Service에 위임할 수 있으며, 하위 Service는 같은 트랜잭션에 참여한다.
...
입력·사용자 검증 → 멱등성 확인/선점 → 메뉴 검증 → 지갑 행 비관적 락
        → 잔액 변경 → 주문·원장·Outbox 저장 → 멱등 완료 응답 저장 → 커밋
```
...
3. 해시가 같으면 저장한 최초 완료 HTTP 상태와 본문을 반환한다.
...
멱등성 행은 `201` 성공과 실행이 시작된 뒤 확정된 결정적 도메인 결과를 커밋한다. 메뉴 없음·비활성, 잔액 부족, 충전 오버플로는 도메인 변경 없이 오류 상태와 본문만 `COMPLETED`로 커밋한다. 구조 검증 실패, 사용자 없음, DB·락·분류되지 않은 서버 오류는 저장하지 않는다. 저장된 실패 조건을 해결하고 재실행하려면 새 키가 필요하다. V1에서는 완료 기록을 자동 삭제하지 않는다.

중복 처리 로직은 트랜잭션 프록시 경계를 분리한다. 유니크 제약 예외로 rollback-only가 된 트랜잭션 안에서 재조회하지 않는다.

## 6. 주문·결제 트랜잭션

Controller는 `OrderFacade`만 호출한다. `OrderFacade`는 트랜잭션 범위와 락 순서를 정의하고 `IdempotencyExecutor`에 실행·재조회 경계를 위임한 뒤, 트랜잭션 콜백 안에서 각 기능의 Service를 조합한다. 각 Service는 자기 Repository를 사용하며 Facade는 Repository나 DB에 직접 접근하지 않는다.
...
sequenceDiagram
    ...
    M->>DB: 메뉴 조회
    O->>PS: 지갑 락·잔액 차감·원장 기록
    PS->>DB: SELECT ... FOR UPDATE와 포인트 쓰기
    O->>OS: PAID 주문 생성
    OS->>DB: 주문 저장
...
1. 짧은 트랜잭션에서 `next_attempt_at <= now`인 `PENDING` 행 또는 `locked_until < now`인 `PROCESSING` 행을 `FOR UPDATE SKIP LOCKED`로 조회한다.
2. 행을 `PROCESSING`으로 바꾸고 `attempt_count`를 증가시키며 매 선점마다 새 UUID `claim_token`, `locked_by`, `locked_until` lease를 기록한 뒤 커밋한다.
3. DB 트랜잭션 밖에서 외부 전송을 수행한다.
4. 결과 갱신은 새 트랜잭션에서 `event_id`, `status = PROCESSING`, 자신이 받은 `claim_token`을 모두 조건으로 수행한다.
5. 성공하면 `PUBLISHED`와 `published_at`을 기록하고 claim·lease를 비운다. 실패하면 오류를 기록하고 claim·lease를 비운 뒤 `PENDING`으로 예약하거나 한도 초과 시 `FAILED`로 바꾼다.
6. 조건부 UPDATE 결과가 0이면 lease를 잃은 작업자의 늦은 결과이므로 무시한다. 만료 행은 이미 다른 작업자가 새 `claim_token`으로 회수했을 수 있다.
...
- 선점 커밋 후 외부 호출 전에 프로세스가 종료되면 `attempt_count` 한 회가 소비될 수 있다. lease 만료 후 다음 주기로 복구하며, 한도 초과 이벤트는 삭제하지 않고 수동 재처리한다.
- 수신자는 `eventId`를 멱등 키로 사용해야 한다.
- `FAILED` 행은 삭제하지 않는다. 운영 확인 후 `attempt_count = 0`, `next_attempt_at = now`, `claim_token = null`, `locked_by = null`, `locked_until = null`, `status = PENDING`으로 원자적으로 바꿔 새 재시도 주기를 시작한다.

따라서 보장은 “성공 시까지 내구성 있게 최소 1회 처리하며, 자동 재시도 한도 후에는 유실시키지 않고 격리·수동 재처리한다”이다. 정확히 한 번 전달을 주장하지 않는다.

## 8. 인기 메뉴 집계

주문 Facade는 주입받은 UTC `Clock`의 한 `Instant`를 `paid_at`과 이벤트 `occurredAt`에 함께 사용한다. 조회 Service도 같은 종류의 UTC `Clock`에서 한 번 `asOf`를 캡처한다. 운영 인스턴스의 시스템 시계는 NTP로 동기화하고, 테스트에서는 고정 `Clock`을 주입한다. 각 요청은 캡처한 하나의 시각만 사용한다.
...
| Outbox 처리 | `SKIP LOCKED` + lease | lease 만료 후 다른 작업자가 회수 |
...
- lease 만료 행이 회수되는지 검증

동시성 테스트는 별도 트랜잭션을 사용하는 스레드와 시작 장벽을 사용한다. 테스트 메서드 자체의 상위 트랜잭션으로 요청을 감싸지 않는다.
...
| [ADR-003](./adr/003-idempotent-write-api.md) | 쓰기 API 멱등성 |
